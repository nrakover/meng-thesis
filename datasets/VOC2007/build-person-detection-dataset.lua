...
local IMAGE_SET_PATH = 'ImageSets/Main/person_trainval.txt'
...
local function getListOfLabeledImageIDs()
	local f = io.open(IMAGE_SET_PATH)
	local images = {}
...
function generateDataset(outfile, numPositiveExamples, numNegativeExamples)
	-- Dataset as VGG features
	local dataset = {data={}, label={}}

	-- Get the IDs of all files labeled for the desired class
	local imageIDs = getListOfLabeledImageIDs()
	...
	for i = 1, #imageIDs do
		...
		if pCount >= numPositiveExamples and nCount >= numNegativeExamples then break end

		if tonumber(imageIDs[i][2]) == 1 or (pCount >= numPositiveExamples and tonumber(imageIDs[i][2]) == -1) then 
			...
			for tagIndx = 1, #annotation do
				...
				if tag:find('object') ~= nil then
					...
					local img_region = img[{{}, {y_min,y_max}, {x_min, x_max}}]
					...
					if tag:find('name')[1] == 'person' then
			...
			if tonumber(imageIDs[i][2]) == -1 and math.random() > 0.6 then
				local imID = imageIDs[i][1]
...
generateDataset('person_dataset.t7', 1000, 1000)
