...
function openImage(img_id)
	local p = io.popen('find "'..IMG_DIR_PATH..'" -type f | grep '..img_id)  
	local file_names = {}
	for file in p:lines() do
	   table.insert(file_names, file)
	end
	if #file_names == 0 then return nil end
	if pcall(function() image.load(file_names[1]) end) then
		return image.load(file_names[1])
	else
		return nil
	end
end
...
for synset_indx = 1,#synsets do
	...
	for img_indx = 1,#attrann.images do
		...
		-- Open image, proceed if successful
		local img = openImage(img_id)
		if img ~= nil then
